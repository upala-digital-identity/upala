...
interface IUpala {
    ...
    // groups
    function newGroup(address groupManager, address poolFactory, address tokenContract) external payable returns (uint160);
    function newPool(address, uint160, address) external payable returns (address);

    // groups (only managers)
    function setGroupManager(uint160, address) external;
    function memberScore(uint160[] calldata) external view returns(uint);
