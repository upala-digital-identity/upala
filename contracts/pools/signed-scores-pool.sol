...
contract SignedScoresPoolFactory {
    ...
    // used for testing
    event NewPool(address poolAddress, address poolManager, address parentFactory);
    ...
    function createPool() external returns (address) {
        ...
        NewPool(newPoolAddress, msg.sender, address(this));  // todo move event to Upala?
...
contract SignedScoresPool is Ownable {
    ...
    ************/
    ...
    function setBaseScore(uint newBaseScore, bytes32 secret) external onlyOwner {
        bytes32 hash = keccak256(abi.encodePacked("setBaseScore", newBaseScore, secret));
    ...
    function increaseBaseScore(uint newBaseScore) external onlyOwner {
        require (newBaseScore > baseScore, "To decrease score, make a commitment first");
        baseScore = newBaseScore;
        NewBaseScore(newBaseScore);
    }
